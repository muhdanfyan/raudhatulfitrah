...
export default function AkademikDashboard() {
  ...
  const canEdit = user?.role && ['superadmin', 'admin', 'akademik'].includes(user.role.toLowerCase());
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (canEdit) {
        ...
        if (rolesRes.success) {
          const roleObj = rolesRes.data.find((r: any) => r.name.toLowerCase() === 'akademik' || r.name.toLowerCase() === 'akademik');
          if (roleObj) setRoleId(roleObj.id);
        }
      }

      // Fetch dashboard data
      const response = await api.getDashboardAkademik();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-1">Dashboard Akademik</h1>
            <p className="text-gray-600 font-medium">Pantau perkembangan akademik santri</p>
          </div>
        </div>
