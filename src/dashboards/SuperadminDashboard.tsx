...
export default function SuperadminDashboard() {
  ...
  const canEdit = user?.role && ['superadmin', 'admin'].includes(user.role.toLowerCase());
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (canEdit) {
        ...
        if (rolesRes.success) {
          const roleObj = rolesRes.data.find((r: any) => r.name.toLowerCase() === 'superadmin' || r.name.toLowerCase() === 'admin');
          if (roleObj) setRoleId(roleObj.id);
        }
      }

      // Fetch dashboard data
      const response = await api.getDashboardAdmin();
    ...
    <div className="space-y-6 text-gray-800">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-1">Selamat Datang, Admin!</h1>
            <p className="text-gray-600 font-medium">Berikut adalah ringkasan sistem pondok pesantren hari ini</p>
          </div>
        </div>
