...
export default function AsramaDashboard() {
  ...
  const canEdit = user?.role && ['superadmin', 'admin', 'akademik', 'pembinaan', 'asrama'].includes(user.role.toLowerCase());
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (canEdit) {
        ...
        if (rolesRes.success) {
          const roleObj = rolesRes.data.find((r: any) => r.name.toLowerCase() === 'asrama');
          if (roleObj) setRoleId(roleObj.id);
        }
      }

      // Fetch dashboard data
      const response = await api.getDashboardAsrama();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-1">Dashboard Asrama</h1>
            <p className="text-gray-600 font-medium">Kelola inventaris, keuangan, dan masukan santri</p>
          </div>
        </div>
